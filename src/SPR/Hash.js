...
let mes = this.text2Binary(message);
let m = this.fillZero(mes + '1',448)  + this.fillLenght(mes);
let w=this.createW(m);
while (w.length<64) {
    w.push('00000000000000000000000000000000');
}
for (let i=16;i<64;i++) {
    let s0 = this.XOR(this.rotateRight(w[i-15],7), this.rotateRight(w[i-15],18), this.Rshift(w[i-15],3));
    let s1 = this.XOR(this.rotateRight(w[i-2],17) ,this.rotateRight(w[i-2],19) , this.Rshift(w[i-2],10));

    w[i]=this.sum(w[i-16],s0);
    w[i]=this.overflowT(w[i]);

    w[i]=this.sum(w[i],w[i-7]);
    w[i]=this.overflowT(w[i]);
    ...
    for (let i=0;i<64;i++) {
        ...
        let t1 = this.sum(this.sum(this.sum(this.sum(this.forK(h.toString(2)),sum2),Ch),this.forK(k[i].toString(2))),w[i]);
        while (t1.length>32) {
            t1=t1.substring(1)
        }
    ...
    this.result = this.fillZeroBegin(h0.toString(16))+" "+this.fillZeroBegin(h1.toString(16))
